class Bird {
  ...
  public void calculateAcceleration(SpatialGrid grid) {
    ...
    /*
    * COHESION - move towards the center of mass of my neighbours
    */
    ...
    for (Bird otherBird : grid.query(position.x, position.y, BIRD_COHESION_RADIUS)) {
      if (otherBird == this) continue;   // Don't compare a bird with itself!
      ...
      // Ignore if too far away
      if (squareDistanceToOtherBird > BIRD_COHESION_RADIUS * BIRD_COHESION_RADIUS) continue;

      // Accumulate center of mass
      centerOfMass.add(otherBird.position);
      ++cohesionNeighbourCount;
    }

    if (cohesionNeighbourCount > 0) {
      centerOfMass.mult(1f / cohesionNeighbourCount);
      cohesionForce = PVector.sub(centerOfMass, position).setMag(BIRD_COHESION_STRENGTH);
